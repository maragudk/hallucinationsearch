...
type resultGenerator interface {
	GenerateResult(ctx context.Context, query string, position int) (llm.Result, error)
}
...
// Per-call randomization inside the LLM layer (archetype / weirdness / reliability signal)
// replaces the old "avoid already-used titles" dedup, which was defeated by parallel fan-out.
func GenerateResult(log *slog.Logger, db generateResultDB, gen resultGenerator, _ *goqite.Queue) jobs.Func {
	return jobs.WithTracing("jobs.GenerateResult", func(ctx context.Context, m []byte) error {
		...
		r, err := gen.GenerateResult(llmCtx, q.Text, jd.Position)
		if err != nil {
